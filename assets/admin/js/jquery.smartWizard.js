...
function SmartWizard(target, options) {
    ...
    var _init = function($this) {
        ...
        var idk = $('#idk').val();
        var idt = $('#id'+$this.curStepIdx).val();
        var ischecked = "";
        var found = prog.some(function (el) {
            return el == idt;
        });
        if(found) ischecked = "checked";
        elmActionBar.append("<span style='margin-right:20px' id='check-content'><input type='checkbox' id='checklist' onclick='setprogres("+idt+","+idk+","+$this.curStepIdx+")' "+ischecked+"> <label for='checklist'>Sudah dilakukan</label> </span> ");
        ...
        $($this.steps).bind("click", function(e){
            ...
            if(isDone == 1){
                var idk = $('#idk').val();
                var idt = $('#id'+nextStepIdx).val();
                var ischecked = "";
                var found = prog.some(function (el) {
                    return el == idt;
                });
                if(found) ischecked = "checked";
                $('#check-content').html("<input type='checkbox' id='checklist' onclick='setprogres("+idt+","+idk+","+nextStepIdx+")' "+ischecked+"> <label for='checklist'>Sudah dilakukan</label> ");
    ...
    var _prepareSteps = function($this) {
        if(! $this.options.enableAllSteps){
            $($this.steps, $this.target).removeClass("selected").removeClass("done").addClass("unselected");
    ...
    var _setupStep = function($this, curStep, selStep) {
        ...
        $(curStep, $this.target).removeClass("unselected");
        ...
        if(found) $(curStep, $this.target).addClass("done");
        else $(curStep, $this.target).addClass("unselected");


        $(selStep, $this.target).removeClass("unselected");
...
// Default Properties and Events
    $.fn.smartWizard.defaults = {
        ...
        labelNext:'Selanjutnya <svg class="glyph stroked chevron right"><use xlink:href="#stroked-chevron-right"/></svg>',
        labelPrevious:'<svg class="glyph stroked chevron left"><use xlink:href="#stroked-chevron-left"/></svg> Sebelumnya',
        labelFinish:'Tutup',
